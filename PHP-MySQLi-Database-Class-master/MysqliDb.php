...
class MysqliDb
{
    ...
    public function insertMultiImproved($tableName, array $multiInsertData, array $dataKeys = null)
    {
        // only auto-commit our inserts, if no transaction is currently running
        $autoCommit = (isset($this->_transaction_in_progress) ? !$this->_transaction_in_progress : true);
        $ids = array();
    ...
    public function insertMultiImprovedDeprecated($tableName, array $multiInsertData, array $dataKeys = null)
    {
        // only auto-commit our inserts, if no transaction is currently running
        if(!is_array($multiInsertData) || count($multiInsertData) < 1)
            return false;
        ...
        $tableKeys = array_keys($multiInsertData[0]);
        ...
        $char = ",".PHP_EOL.PHP_EOL;
        $query = rtrim($query,$char);
